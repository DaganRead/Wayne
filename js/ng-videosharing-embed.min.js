/*! ng-videosharing-embed - v0.3.7 - Embed videos using AngularJS directives - built 2017-01-09 - License MIT (http://www.opensource.org/licenses/MIT) */
...
}), angular.module("videosharing-embed").directive("embedVideo", ["$filter", "RegisteredPlayers", "$sce", "$window", function(a, b, c, d)
{
	...
	return {
		...
		{
			var h = void 0,
				i = angular.isString(g.forceProtocol) ? g.forceProtocol + "://" : void 0;
			...
			}), g.$observe("href", function(f)
			{
				if (void 0 !== f && f !== h)
				{
					...
					if (angular.forEach(b, function(a)
						{
							a.isPlayerFromURL(f) && (j = a)
						}), null === j) return void e.onChange();
					var k = f.match(j.playerRegExp);
					i = i || k[1];
					var l = k[2],
						m = f.match(j.timeRegExp);
					j.config;
					if (angular.forEach(a("whitelist")(g, j.whitelist), function(a, b)
						{
							var c = void 0 != j.transformAttrMap[b] ? j.transformAttrMap[b] : b;
							j.settings[c] = a
						}), j.settings.start = 0, m) switch (j.type)
					{
						case "youtube":
							j.settings.start += 60 * parseInt(m[2] || "0") * 60, j.settings.start += 60 * parseInt(m[4] || "0"), j.settings.start += parseInt(m[6] || "0");
							break;
						case "dailymotion":
							j.settings.start += parseInt(m[1] || "0")
					}
					if (j.isAdditionaResRequired())
						for (var n = angular.element(d.document.querySelector("body")), o = 0; o < j.additionalRes.length; o++)
						{
							var p = j.additionalRes[o];
